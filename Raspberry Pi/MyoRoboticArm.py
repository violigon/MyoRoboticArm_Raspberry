...
def setInterval(funcao, intervalo):
    def funcaoWrapper():
        setInterval(funcao, intervalo)
        funcao()

    t = threading.Timer(intervalo, funcaoWrapper)
    t.start()
    
    return t

def setTimeout(funcao):
    t = threading.Thread(funcao)
    t.start()
...
def Fist():
    threading.Thread(target=SetAngle, args=(motor1, 0)).start()
    threading.Thread(target=SetAngle, args=(motor2, 0)).start()
    threading.Thread(target=SetAngle, args=(motor3, 180)).start()
    time.sleep(0.5)
    threading.Thread(target=SetAngle, args=(motor1, 80)).start()
    
    
def FingersSpread():
    threading.Thread(target=SetAngle, args=(motor1, 0)).start()
    time.sleep(0.5)
    threading.Thread(target=SetAngle, args=(motor2, 165)).start()
    threading.Thread(target=SetAngle, args=(motor3, 0)).start()
...
if __name__ == '__main__':
    ...
    m.add_pose_handler(lambda p: ProcessarPose(p))
